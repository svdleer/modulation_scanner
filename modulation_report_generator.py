    ...
    def get_modulation_data_optimized(self):
        ...
        try:
            ...
            if self.sqlalchemy_engine:
                ...
                df = pd.read_sql(base_query, self.sqlalchemy_engine)
            else:
...
Deze e-mail is automatisch verzonden"""
    ...
    def run_web_only_report(self, report_date=None):
        """Generate web JSON report without email, and copy as normal file"""
        if report_date is None:
            report_date = datetime.now().strftime('%Y-%m-%d')
        ...
        try:
            ...
            if json_file:
                # Copy the web JSON file as a normal file (without _web suffix)
                normal_json_file = os.path.join(self.output_dir, f"modulation_report_{report_date}.json")
                
                import shutil
                shutil.copy2(json_file, normal_json_file)
                self.logger.info(f"Copied web JSON to normal file: {normal_json_file}")
                
                self.logger.info("Web-only report generation completed successfully")
                
                return {
                    'web_json_file': json_file,
                    'normal_json_file': normal_json_file,
                    'report_date': report_date,
                    'success': True
                }
            else:
                self.logger.error("Failed to generate web JSON file")
                return {
                    'success': False,
                    'error': 'Failed to generate web JSON file',
                    'report_date': report_date
                }
...
if __name__ == "__main__":
    ...
    Usage:
        ...
        python modulation_report_generator.py --web-only        # Generate web JSON and copy as normal file, no email
    """
